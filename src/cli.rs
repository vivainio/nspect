    ...
    match &args.output_dir {
        ...
        Some(dir) => {
            ...
            write_artifact(
                &dir.join("AGENTS.md"),
                "",
                &agents_md(ext, args.check, references_snapshot.is_some()),
            )?;
        }
    }
    Ok(())
...
/// Orientation for an agent that lands in this directory cold: what's here,
/// and that `nspect lookup` beats grepping these files directly.
fn agents_md(ext: &str, has_checks: bool, has_references: bool) -> String {
    let mut out = String::from(
        "\
# AGENTS.md — `.nspect/gen/`

Generated by `nspect init` / `nspect atlas --output-dir`: a structural
snapshot of this repo, meant for agents to query instead of grepping
source. Regenerated on every run and gitignored — do not hand-edit.

Prefer `nspect lookup <TypeName>` (or `nspect lookup --file <path>`) over
reading these files directly: it resolves a type or file to its declaring
project, namespace, metrics, and cross-project callers in one shot. Each
file below also carries its own header comment describing its shape.

Files:
",
    );
    out.push_str(&format!(
        "- `atlas.{ext}` — structural snapshot: areas, projects, refs, cycles, layers.\n"
    ));
    out.push_str(&format!(
        "- `classes.{ext}` — types declared per project, grouped by namespace/kind.\n"
    ));
    out.push_str(&format!(
        "- `metrics.{ext}` — per-type/per-method structural metrics with source spans.\n"
    ));
    if has_checks {
        out.push_str(&format!(
            "- `checks.{ext}` — findings from `atlas --check` (cycles, unresolved refs, version conflicts, ...).\n"
        ));
    }
    if has_references {
        out.push_str(&format!(
            "- `references.{ext}` — cross-project type references (resolved/ambiguous/external).\n"
        ));
    }
    out.push_str(&format!(
        "- `tips.{ext}` — non-authoritative structural suggestions (merge/cluster candidates).\n"
    ));
    out.push_str(&format!(
        "- `endpoints.{ext}` — types exposing/consuming a remote interface (WCF/Web API/SignalR).\n"
    ));
    out.push_str(
        "- `build-deps.json` — always JSON; project build-dependency edges for build tooling.\n\
         - `build-plan.json` — always JSON; suggested build order for build tooling.\n",
    );
    out
}
